...
import { Course, CourseDB } from "../models/Course"
...
export class CourseBusiness{
    ...
    public getCourses = async (input: any) => {
        const { q } = input
        const coursesDB = await this.courseDatabase.findCourse(q)
    ...
    public editCourse = async (input: EditCourseInputDTO): Promise<EditCourseOutputDTO> => {
        ...
        await this.courseDatabase.editCourse(idToEdit, updateCourseDB)
